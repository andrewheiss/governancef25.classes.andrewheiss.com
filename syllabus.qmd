---
title: "Syllabus"
pdf-title: "Governance Around the World"
pdf-subtitle: "PMAP 8441/4441 • Fall 2025"
pdf-note: |
  The full version of the course syllabus, schedule, and all course materials is available online at <https://governancef25.classes.andrewheiss.com/>. This is only a partially complete static version.
pdf-header-left: PMAP 8441/4441
pdf-header-right: Fall 2025
pdf-footer-left: Governance Around the World
reference-location: margin
format: 
  html: default
  typst:
    # COURSE SUBJECT-COURSE NUMBER-FACULTY LAST NAME-CRN-TERMYEAR
    output-file: "PMAP-8441-HEISS-90387-FA25.pdf"
    citeproc: true
...
You're also expected to use quality news sources (e.g. *The New York Times*, *The Washington Post*, *The Economist*, *The Wall Street Journal*, *The Guardian*, or high quality blogs or online publications) to keep up with current events and policy debates related to the class. I also recommend [subscribing to Don Moynihan's "Can We Still Govern?" newsletter](https://donmoynihan.substack.com/).
...
I want you to learn lots of things from this class, but I primarily want you to stay healthy, balanced, and grounded during these chaotic, awful times.
...
Please watch [this video](https://vimeo.com/270014784):
...
schedule <- read_csv(here::here("data/schedule.csv")) |>
  mutate(
    ...
    date_nice = format(date, format(date, "%B %e"))
  ) |>
  mutate(
    title = case_when(
      str_detect(class, "class") ~ glue::glue("*{title}*"),
      !is.na(deadline) ~ glue::glue("{title} _(due by {deadline_nice})_"),
      .default = title
    )
  ) |>
  select(group, subgroup, date_nice, title)
...
tbl_data <- schedule |>
  ...
  mutate(
    subgroup_sequential = cumsum(is_first),
    subgroup_highlight = subgroup_sequential %% 2
  )

# Extract the rows that should be highlighted
highlight_rows <- tbl_data |>
  filter(subgroup_highlight == 1) |>
  pull(row_id)

# Make the table
tbl_data |>
  select(subgroup, date_nice, title) |>
  tt(colnames = FALSE) |>
